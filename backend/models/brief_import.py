...
class FieldExtraction(BaseModel):
    ...
    source: Optional[str] = Field(None, description="Source location in file (e.g., 'line 42')")
...
class ParsedBriefResponse(BaseModel):
    ...
    warnings: List[str] = Field(
        default_factory=list, description="Non-critical warnings during parsing"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Metadata about the parsing operation"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                "success": True,
                "fields": {
                    "companyName": {"value": "Acme Corp", "confidence": "high", "source": "line 3"},
                    "businessDescription": {
...
class ParseError(BaseModel):
    ...
    code: Literal[
        "INVALID_FILE_TYPE", "FILE_TOO_LARGE", "ENCODING_ERROR", "PARSE_FAILED", "NETWORK_ERROR"
